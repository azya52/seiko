	...
	##define piceAngle 16
	##define piceNum 17
	##define piceSelecter 18
	##define piceShift 19
	##define scoresAddL 20
	;21;22
	##define scoresAddH 23
	##define curPosL 20
	##define curPosH 21
	...
	##define brdAdr2b ((((board & 0x3FF)*2)>>8) & 0x0F)
	##define brdAdr1b ((((board & 0x3FF)*2)>>4) & 0x0F)
	##define brdAdr0b ((((board & 0x3FF)*2)   ) & 0x0F)
	
	##define picesAdr2b ((((pices & 0x3FF)*2)>>8) & 0x0F)
	##define picesAdr1b ((((pices & 0x3FF)*2)>>4) & 0x0F)
	##define picesAdr0b ((((pices & 0x3FF)*2)   ) & 0x0F)
...
; проблема со смещением у транслятора, следить чтоб указатели всегдна на четный байт попадали
dsTETRIS:
	DS "TETRIS"
dsGAMEOVER:
	DS "GAME OVER!"
